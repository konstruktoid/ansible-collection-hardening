...
| `sshd_disable_forwarding` | `true` when every individual forwarding option is disabled and no `sshd_match_*` rule mentions forwarding, `false` otherwise | Disables all forwarding, and overrides every other forwarding related directive, including the ones set inside a Match block. Requires OpenSSH 7.4 or later. |
