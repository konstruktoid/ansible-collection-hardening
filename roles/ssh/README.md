...
| `sshd_disable_forwarding` | `true` when every individual forwarding option is disabled and no `sshd_match_*` rule mentions forwarding, `false` otherwise | Disables all forwarding, and overrides every other forwarding related directive, including the ones set inside a Match block. Requires OpenSSH 7.4 or later. Since `DisableForwarding` does not disable agent and X11 forwarding before OpenSSH 10.0 ([CVE-2025-32728](https://nvd.nist.gov/vuln/detail/CVE-2025-32728)), enabling this writes `GatewayPorts`, `PermitTunnel`, `AllowTcpForwarding`, `AllowAgentForwarding` and `X11Forwarding` as disabled as well, and the role fails if a `sshd_match_*` rule mentions forwarding. |
...
| `sshd_per_source_max_startups` | `10` | Specifies the maximum number of unauthenticated connections permitted from any single source address, in line with the burst allowance of `sshd_max_startups`. Requires OpenSSH >= 9.8, ignored on older versions. |
